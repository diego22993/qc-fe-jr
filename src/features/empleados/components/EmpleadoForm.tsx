...
const FilaEmpleado = ({ empleado }: { empleado: Empleado }) => (
  <tr className="border-t border-neutral-100 hover:bg-neutral-50 transition-colors">
    ...
    <td className="px-4 py-3"><RolBadge rol={empleado.rol} /></td>
    <td className="px-4 py-3 text-neutral-600">{empleado.email}</td>
    <td className="px-4 py-3 text-neutral-600">{empleado.telefono}</td>
    <td className="px-4 py-3 text-neutral-600">{formatFecha(empleado.fechaIngreso)}</td>
    <td className="px-4 py-3 text-neutral-600">${empleado.sueldo.toLocaleString('es-AR')}</td>
    <td className="px-4 py-3"><EstadoBadge active={empleado.active} /></td>
  </tr>
);
  ...
  if (empleados.length === 0) {
    return (
      <div className="flex items-center justify-center py-16">
        <p className="text-neutral-400 text-sm">No hay empleados registrados todavía.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border border-neutral-100 rounded-lg overflow-hidden">
      <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
        <h2 className="text-sm font-medium text-neutral-800">Empleados registrados</h2>
        <span className="text-xs bg-neutral-50 border border-neutral-100 px-3 py-1 rounded-full text-neutral-600">
          {empleados.length} {empleados.length === 1 ? 'registro' : 'registros'}
        </span>
      </div>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-neutral-50">
              <th className="text-left px-5 py-3 font-medium text-neutral-600">Nombre</th>
              ...
              <th className="text-left px-4 py-3 font-medium text-neutral-600">Ingreso</th>
              <th className="text-left px-4 py-3 font-medium text-neutral-600">Sueldo</th>
              <th className="text-left px-4 py-3 font-medium text-neutral-600">Estado</th>
            </tr>
          </thead>
          <tbody>
            {empleados.map(emp => (
              <FilaEmpleado key={emp.id} empleado={emp} />
            ))}
          </tbody>
        </table>
      </div>
